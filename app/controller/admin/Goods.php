...
class Goods extends AdminBase
{
   ...
   public function lst()
   {
       ...
       $flowerMaterial = \app\model\FlowerMaterial::where('is_delete', '=', 1)->column('title', 'id');
       $flowerUse = \app\model\FlowerUse::where('is_delete', '=', 1)->column('title', 'id');
       $ret = [
           'data'           => $data,
   ...
   public function getGoodsImgAttr($value)
   {
    ...
    public function edit()
    {
        ...
        $flowerMaterialIds = empty($data['flower_material']) ? [] : explode(',', $data['flower_material']);
        $flowerUse = \app\model\FlowerUse::where('is_delete', '=', 1)->column('title', 'id');
        $flowerUseIds = empty($data['flower_use']) ? [] : explode(',', $data['flower_use']);

        $introduceImg = \app\model\GoodsIntroduceImg::where(['goods_id' => $id, 'is_delete' => 1])->select();
        $ret = [
            'data'               => $data,
            'flowerMaterial'     => $flowerMaterial,
            'flowerMaterialIds'  => $flowerMaterialIds,
            'flowerUse'          => $flowerUse,
            'flowerUseIds'       => $flowerUseIds,
